...
function formatCompileError(error: CompileError): string {
  const pathMessage = `${chalk.cyan(error.location.file)}:${chalk.yellow(
    error.location.line,
  )}:${chalk.yellow(error.location.column)}`
  const categoryMessage = chalk.red('error:')

  const code = `${chalk.gray(error.location.line)} ${error.location.lineText}${
    os.EOL
  }${repeatString(' ', error.location.column + `${error.location.line}`.length + 1 + 1)}${chalk.red(
    repeatString('~', error.location.length),
  )}${repeatString(
    ' ',
    error.location.lineText.length - error.location.column - error.location.length,
  )}`
